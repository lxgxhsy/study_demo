...
- [ ] Task submission APIs return `acceptedCount`, `rejectedCount`, `callerRunsCount`, `submittedCount`, and `requestId`.
...
- [ ] The response includes `orderId`, `acceptedAsyncTasks`, `rejectedAsyncTasks`, `callerRunsAsyncTasks`, and `requestId`.
...
- [ ] Waiting for an async preload is bounded by `preload-wait-timeout-ms`.
